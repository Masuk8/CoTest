...
internal class BundleConfigurationProvider: ConfigurationProvider {
    internal static var plistName: String = Consts.defaultPlistName
    ...
    private struct Consts {
        static let defaultPlistName = "Copilot-Info"
    ...
    init(bundle: Bundle = .main) {
        guard let plistFile = bundle.path(forResource: BundleConfigurationProvider.plistName, ofType: Consts.plistType) else {
            fatalError("\(BundleConfigurationProvider.plistName).plist file not found.")
        }
        ...
        guard let copilotManageType = configurationManageType else {
            fatalError("\(BundleConfigurationProvider.plistName).plist file is missing the mandatory `MANAGE_TYPE` key, it should point to a String value.")
        }
        ...
        if copilotManageType.lowercased() == Consts.yourOwn.lowercased() {
            ...
            fatalError("\(BundleConfigurationProvider.plistName).plist file is missing the mandatory `MANAGE_TYPE` key, it should point to a String value `YourOwn` or `CopilotConnect`.")
        }
