...
internal extension UIView {
    ...
    func setHeight(_ height: CGFloat) {
        resetPreviousConstraints(ofType: heightAnchor)
        heightAnchor.constraint(equalToConstant: height).isActive = true
    }

    func setHeight(equalToConstraint heightConstraint: NSLayoutDimension) {
        resetPreviousConstraints(ofType: heightAnchor)
        heightAnchor.constraint(equalTo: heightConstraint).isActive = true
    }

    func setWidth(_ width: CGFloat) {
        resetPreviousConstraints(ofType: widthAnchor)
        widthAnchor.constraint(equalToConstant: width).isActive = true
    }

    func setWidth(equalToConstraint widthConstraint: NSLayoutDimension) {
        resetPreviousConstraints(ofType: widthAnchor)
        widthAnchor.constraint(equalTo: widthConstraint).isActive = true
    }

    fileprivate func resetPreviousConstraints(ofType type: NSLayoutDimension) {
        translatesAutoresizingMaskIntoConstraints = false
        for constraint in constraints {
            guard constraint.firstAnchor == type else { continue }
